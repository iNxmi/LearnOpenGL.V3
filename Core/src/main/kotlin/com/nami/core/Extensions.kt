...
fun Vector2i.asID(size: Vector2i): Long {
    var id = 0L
    id += this.x
    id += this.y * size.x
...
fun Vector3i.asID(size: Vector3i): Long {
    var id = 0L
    id += this.x
    id += this.y * size.x
    id += this.z * size.x * size.y

    return id
}

fun Long.idToVector2i(size: Vector2i): Vector2i {
    val y = (this / size.x).toInt()
    val x = (this % size.x).toInt()

    return Vector2i(x, y)
}

fun Long.idToVector3i(size: Vector3i): Vector3i {
    val z = (this / (size.x * size.y)).toInt()
    val zRemainder = this % (size.x * size.y)
    val y = (zRemainder / size.x).toInt()
    val x = (zRemainder % size.x).toInt()
