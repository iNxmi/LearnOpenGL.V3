...
class ResourceLoaderItem : Resources<Item>(GamePath.item, "item", arrayOf("json")) {
    ...
    override fun onLoad(id: String, path: Path): Item {
        ...
        val handlerPath = when (itemPath[0]) {
            "block" -> "com.nami.world.inventory.item.handlers.ItemBlockHandler"
            "tool" -> "com.nami.world.inventory.item.handlers.ItemToolHandler"
            else -> "com.nami.world.inventory.item.handlers.ItemHandler${id.snakeToUpperCamelCase()}"
        }

        var handlerClass: Class<*> = try {
            Class.forName(handlerPath)
        } catch (e: Exception) {
            Class.forName("com.nami.world.inventory.item.handlers.DefaultItemHandler")
        }
